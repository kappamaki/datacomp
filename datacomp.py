...
def series_mismatch_indices(series, other):
    # Get indices where value is null in one series and not the other
    mismatch_null_idx = set(series[series.isnull()].index) ^ set(other[other.isnull()].index)
    # Get indices where values are not null and do not match
    notnull_idx = set(series[series.notnull()].index) & set(other[other.notnull()].index)
    mismatch_notnull_idx = series.loc[notnull_idx] != other.loc[notnull_idx]
    mismatch_notnull_idx = set(series.loc[notnull_idx][mismatch_notnull_idx].index)
    # Combine null and non-null mismatches
    return mismatch_null_idx | mismatch_notnull_idx
    ...
    if not df1[common_cols].equals(df2[common_cols]):
        ...
        for idx, col in enumerate(common_cols):
            ...
            if not df1[col].equals(df2[col]):
                ...
                mismatch_idx = series_mismatch_indices(df1[col], df2[col])
